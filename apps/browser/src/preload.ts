...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  webContents: {
    ...
    onDidStartLoading: (callback: () => void) => {
      ...
      return () => ipcRenderer.removeListener('webcontents-did-start-loading', callback);
    },
    onDidStopLoading: (callback: () => void) => {
      ipcRenderer.on('webcontents-did-stop-loading', callback);
      return () => ipcRenderer.removeListener('webcontents-did-stop-loading', callback);
    },
    onDidNavigate: (callback: (url: string) => void) => {
      ...
      return () => ipcRenderer.removeListener('webcontents-did-navigate', listener);
    },
    onDidNavigateInPage: (callback: (url: string) => void) => {
      const listener = (_event: any, url: string) => callback(url);
      ipcRenderer.on('webcontents-did-navigate-in-page', listener);
      return () => ipcRenderer.removeListener('webcontents-did-navigate-in-page', listener);
    },
    onDomReady: (callback: () => void) => {
      ipcRenderer.on('webcontents-dom-ready', callback);
      return () => ipcRenderer.removeListener('webcontents-dom-ready', callback);
    },
    onDidFailLoad: (callback: (errorCode: number, errorDescription: string) => void) => {
      const listener = (_event: any, errorCode: number, errorDescription: string) => 
        callback(errorCode, errorDescription);
      ipcRenderer.on('webcontents-did-fail-load', listener);
      return () => ipcRenderer.removeListener('webcontents-did-fail-load', listener);
    },
    onRenderProcessGone: (callback: (details: any) => void) => {
      ...
      return () => ipcRenderer.removeListener('webcontents-render-process-gone', listener);
    },
  },
});
